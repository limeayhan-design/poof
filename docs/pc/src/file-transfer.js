...
export class FileTransfer extends EventTarget {
  ...
  _handleEnvelope(env) {
    // iOS emits UUIDs uppercase (`UUID().uuidString`), but decoded binary
    // frames come out lowercase (`bytesToUuid`). Normalize here so envelope-
    // keyed state matches frame-keyed lookups.
    const id = typeof env.id === 'string' ? env.id.toLowerCase() : env.id;
    if (env.type === PoofMessageType.FileMeta) {
      const { name, size, mime, chunks } = env.payload;
      const existing = this.incoming.get(id);
      if (existing && !existing.provisional) {
        console.log('[file-transfer] fileMeta duplicate (resume?)', id, name);
      ...
      this.dispatchEvent(new CustomEvent('incoming-meta', {
        detail: { id, name, size, mime },
      }));
    } else if (env.type === PoofMessageType.FileCancel) {
      if (this.incoming.delete(id) || this.outgoing.delete(id)) {
        this.dispatchEvent(new CustomEvent('cancelled', {
          detail: { id, reason: env.payload?.reason || 'peer' },
        }));
      }
      ...
      this.dispatchEvent(new CustomEvent('delivered', { detail: { id } }));
    } else if (env.type === PoofMessageType.FileSeen) {
      this.dispatchEvent(new CustomEvent('seen', { detail: { id } }));
    }
  }
  ...
  _handleFrame(frame) {
    ...
    if (frame.isLast) {
      // If provisional (fileMeta hasn't landed yet), give the control channel
      // a short window to catch up — small files finish before their meta
      // envelope arrives on the separate channel. Otherwise finalize now.
      if (state.provisional) {
        state.finalizing = true;
        setTimeout(() => this._finalizeIncoming(frame.transferId), 500);
      } else {
        this._finalizeIncoming(frame.transferId);
      }
    }
  }
  ...
  _finalizeIncoming(transferId) {
    ...
    if (!state) return;
    if (state.provisional) {
      // fileMeta never arrived — synthesize name from sniffed magic bytes.
    ...
    this.dispatchEvent(new CustomEvent('completed', {
      detail: { id: transferId, meta: state.meta, blob },
    }));
  }
