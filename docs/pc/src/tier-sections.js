...
//   - callbacks: pushClipboard, openFolderPicker, openRemoteFiles,
...
const svg = {
  ...
  server: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="6" rx="1.5"/><rect x="3" y="14" width="18" height="6" rx="1.5"/><path d="M7 7h.01M7 17h.01"/></svg>',
};
...
function renderDevium(root, ctx) {
  ...
  const peers = ctx.peers || [];
  const onlineCount = peers.filter((p) => ctx.isPeerOnline?.(p.id)).length;
  const airgap = ctx.airGapState || { enabled: false, signalingIsLAN: true };
  const multiDropDisabled = onlineCount === 0;
  const rtcConnected = !!ctx.isRTCConnected;
  const signalingConnected = !!ctx.isSignalingConnected;
  const connState = ctx.connectionState || (rtcConnected ? 'connected' : (signalingConnected ? 'waiting for peer…' : 'offline'));
  const signalingURL = ctx.signalingURL || '';
  ...
  root.innerHTML = `
    ...
    <button class="ts-card" data-action="multi-drop" ${multiDropDisabled ? 'disabled' : ''} style="--tier-accent:${accent};opacity:${multiDropDisabled && peers.length ? '0.6' : '1'}">
      <div class="ts-card-head">
        <span class="ts-card-title-line">
          <span class="ts-card-title">Multi-Drop</span>
        </span>
        <span class="ts-card-sub">${peers.length === 0
            ? 'No devices paired'
            : multiDropDisabled ? 'No device online'
            : `Broadcast to ${onlineCount}/${peers.length} online`}</span>
      </div>
      ${multiDrop}
    </button>

    <button class="ts-cli" data-action="debug-console" style="--tier-accent:${accent}">
      <div class="ts-cli-head">
        ...
        <span class="ts-cli-tag">Debug console</span>
      </div>
      <div class="ts-cli-line">
        <span class="ts-cli-prompt" style="color:${rtcConnected ? '#3FBF7F' : '#8B95A7'}">●</span>
        <span class="ts-cli-cmd">${esc(connState)}</span>
        <span class="ts-cli-count" style="color:${accent}">${onlineCount} online</span>
      </div>
    </button>

    <button class="ts-row" data-action="self-host" style="--tier-accent:${accent}">
      <span class="ts-row-icon" style="color:${accent};background:${accent}28">${svg.server || svg.key}</span>
      <span class="ts-row-body">
        <span class="ts-row-title-line">
          <span class="ts-row-title">Self-hosted signaling</span>
        </span>
        <span class="ts-row-sub"><code>${esc(signalingURL)}</code></span>
      </span>
      <span class="ts-row-arrow" style="color:#8B95A7">${svg.chevronRight}</span>
    </button>

    <label class="ts-row" data-action="airgap-toggle" style="--tier-accent:${accent};cursor:pointer">
      ...
      <span class="ts-row-body">
        ...
        <span class="ts-row-sub" style="color:${airgap.enabled && !airgap.signalingIsLAN ? '#FFBF3C' : ''}">
          ${airgap.enabled
              ? (airgap.signalingIsLAN ? 'LAN only · no internet' : 'LAN only · signaling server is not LAN')
              : 'Internet enabled'}
        </span>
      </span>
      <input type="checkbox" class="kid-switch" id="devium-airgap" ${airgap.enabled ? 'checked' : ''}/>
    </label>
  `;

  root.querySelector('[data-action="multi-drop"]')?.addEventListener('click', () => {
    if (peers.length === 0) {
      ctx.openFeaturePreview?.({
        icon: '🔲', title: 'Multi-Drop', tagline: 'Broadcast a file to N devices at once',
        status: 'available', accent,
        description: 'Pair a few devices from the gear menu, then send one file to all of them in a single click. WebRTC peer-to-peer, no cloud relay.',
      });
      return;
    }
    if (multiDropDisabled) { ctx.toast?.('No device online'); return; }
    ctx.multiDrop?.();
  });
  root.querySelector('[data-action="debug-console"]')?.addEventListener('click', () => ctx.openDebugConsole?.());
  root.querySelector('[data-action="self-host"]')?.addEventListener('click', () => ctx.openSelfHost?.());
  root.querySelector('#devium-airgap')?.addEventListener('change', (e) => {
    ctx.onAirGapToggle?.(e.target.checked);
  });
}
...
function renderBusiness(root, ctx) {
  const tierAccent = TierMeta.business.accent;
  const ws = ctx.workspaceState || { name: '', logo: 'P', color: tierAccent };
  const accent = ws.color || tierAccent;
  const glow = TierMeta.business.glow;
  const logo = ws.logo || 'P';
  const peers = ctx.peers || [];
  const memberCount = peers.length + 1;
  const colors = ['#8FA3B5', '#5B8BFF', '#8B7EFF', '#3FBF7F', '#FF7A9C'];
  const workspaceName = ws.name || `${ctx.deviceName || 'Your'}'s workspace`;
  ...
  root.innerHTML = `
    ...
    <button class="ts-row" data-action="workspace" style="--tier-accent:${accent}">
      <span class="ts-workspace-logo" style="background:linear-gradient(135deg,${accent},${accent}b3)">${esc(logo)}</span>
      <span class="ts-row-body">
        <span class="ts-row-title-line">
    ...
    <button class="ts-row" data-action="branding" style="--tier-accent:${accent}">
      ...
      <span class="ts-row-body">
        ...
        <span class="ts-row-sub">Logo · ${esc(logo)} · color · ${esc(String(accent).toUpperCase())}</span>
      </span>
      <span class="ts-row-arrow" style="color:#8B95A7">${svg.chevronRight}</span>
    </button>
  `;

  root.querySelector('[data-action="workspace"]')?.addEventListener('click', () => ctx.openWorkspace?.());
  root.querySelector('[data-action="live-audit"]')?.addEventListener('click', () => ctx.openAudit?.());
  root.querySelector('[data-action="manage-team"]')?.addEventListener('click', (e) => {
    e.stopPropagation();
    ctx.openTeam?.();
  });
  root.querySelector('[data-action="branding"]')?.addEventListener('click', () => ctx.openWorkspace?.());
}
