...
import * as Workspace from './src/business-workspace.js';
...
const SIGNALING_URL_KEY = 'poof.signalingURL';
const SIGNALING_URL = localStorage.getItem(SIGNALING_URL_KEY)
  || window.ATLAS_SIGNALING_URL
  || location.origin;
...
lan.addEventListener('peer-found', ({ detail }) => {
  console.log('[lan] peer-found event received:', detail);
  ...
  console.log('[lan] store.upsert done. store.peers now:', store.peers.map((p) => p.id));
  ...
  renderDevices();
  console.log('[lan] renderDevices called');
});
...
console.log('[lan] boot: isAvailable=', lan.isAvailable, 'deviceId=', DeviceIdentity.deviceId, 'name=', DeviceIdentity.name);
if (lan.isAvailable) {
  lan.start({
    deviceId: DeviceIdentity.deviceId,
    name:     DeviceIdentity.name,
  })
    .then(() => console.log('[lan] boot: start() resolved, advertising as', DeviceIdentity.deviceId))
    .catch((e) => console.warn('[lan] boot: start() failed:', e));
}
...
// ---- Air-gap (Devium tier, on-device only) --------------------------

const AIRGAP_KEY = 'poof.airgap.enabled';
...
function airGapState() {
  return {
    enabled: localStorage.getItem(AIRGAP_KEY) === '1',
    signalingIsLAN: isLANHost(SIGNALING_URL),
  };
}

function setAirGapEnabled(on) {
  localStorage.setItem(AIRGAP_KEY, on ? '1' : '0');
  if (on && !isLANHost(SIGNALING_URL)) {
    toast('Air-gapped: signaling server is not on your LAN — disconnecting');
    signaling.disconnect?.();
    manager.close?.();
  } else if (!on) {
    toast('Air-gapped mode off');
    signaling.connect?.();
  } else {
    toast('Air-gapped mode on · LAN only');
  }
  renderTierUI();
}

// Enforce at boot: if air-gap was left on and server is non-LAN, don't connect.
if (localStorage.getItem(AIRGAP_KEY) === '1' && !isLANHost(SIGNALING_URL)) {
  // The SignalingClient may auto-connect in its constructor; ask it to stop.
  setTimeout(() => { signaling.disconnect?.(); }, 0);
}

// ---- Self-hosted signaling (Devium tier, on-device only) ------------

function updateSignalingURLAndReload(url) {
  const trimmed = String(url || '').trim();
  if (!trimmed) { toast('URL is empty'); return false; }
  let u;
  try { u = new URL(trimmed); if (!u.host) throw new Error('bad'); }
  catch { toast('Invalid URL (try https://host:3000)'); return false; }
  if (airGapState().enabled && !isLANHost(u.href)) {
    toast('Air-gap refuses non-LAN URLs');
    return false;
  }
  localStorage.setItem(SIGNALING_URL_KEY, u.href);
  toast('Signaling URL saved · reloading');
  setTimeout(() => location.reload(), 400);
  return true;
}

function resetSignalingURL() {
  localStorage.removeItem(SIGNALING_URL_KEY);
  toast('Signaling URL reset · reloading');
  setTimeout(() => location.reload(), 400);
}
...
// ---- Devium: Debug console sheet ------------------------------------

let _debugRepaint = null;

function openDebugConsoleSheet() {
  let dlg = document.getElementById('modal-debug');
  if (!dlg) {
    dlg = document.createElement('dialog');
    dlg.id = 'modal-debug';
    dlg.className = 'sheet debug-sheet';
    dlg.innerHTML = `
      <form method="dialog" class="sheet-form">
        <header class="sheet-head">
          <h2 class="sheet-title">Debug console</h2>
          <button class="sheet-close" value="cancel" aria-label="Close">✕</button>
        </header>
        <div class="debug-body" id="debug-body">Loading…</div>
      </form>
    `;
    document.body.appendChild(dlg);
    dlg.querySelector('.sheet-close').addEventListener('click', () => dlg.close('cancel'));
    dlg.addEventListener('close', () => {
      if (_debugRepaint) { clearInterval(_debugRepaint); _debugRepaint = null; }
    });
  }
  const body = dlg.querySelector('#debug-body');
  const paint = () => { body.innerHTML = renderDebugConsole(); wireDebugCopies(body); };
  paint();
  if (_debugRepaint) clearInterval(_debugRepaint);
  _debugRepaint = setInterval(paint, 500);
  dlg.showModal();
}

function renderDebugConsole() {
  const ag = airGapState();
  const paired = store.peers || [];
  const onlineIds = new Set(state.online);
  const active = state.connectedPeerId || null;
  const rtcState = manager?.state || (manager?.isReady ? 'connected' : 'idle');
  const rows = (label, value, opts = {}) => `
    <div class="debug-row">
      <span class="debug-lbl">${label}</span>
      <span class="debug-val ${opts.mono ? 'mono' : ''}">${escapeHtml(value)}</span>
      ${opts.copy ? `<button class="debug-copy" data-copy="${escapeHtml(value)}">⧉</button>` : ''}
    </div>`;
  const dot = (ok, okColor = '#3FBF7F', badColor = '#FF3B30') =>
    `<span class="debug-dot" style="background:${ok ? okColor : badColor}"></span>`;
  const peerList = paired.length === 0
    ? `<div class="debug-empty">No paired devices.</div>`
    : paired.map((p) => {
        const on = onlineIds.has(p.id);
        return `
          <div class="debug-peer">
            ${dot(on, '#3FBF7F', '#8B95A7')}
            <span class="debug-peer-name">${escapeHtml(p.name)}</span>
            <span class="debug-peer-id">${escapeHtml(p.id)}</span>
            <button class="debug-copy" data-copy="${escapeHtml(p.id)}">⧉</button>
          </div>`;
      }).join('');
  return `
    <section class="debug-section">
      <div class="debug-head">SIGNALING</div>
      ${rows(signalingConnected ? '● connected' : '○ disconnected', SIGNALING_URL, { mono: true, copy: true })}
      ${rows('air-gap', ag.enabled ? 'on · LAN only' : 'off')}
    </section>
    <section class="debug-section">
      <div class="debug-head">WEBRTC</div>
      ${rows('state', rtcState, { mono: true })}
      ${rows('active peer', active || '—', { mono: true, copy: !!active })}
    </section>
    <section class="debug-section">
      <div class="debug-head">PEERS · ${onlineIds.size}/${paired.length} online</div>
      ${peerList}
    </section>
    <section class="debug-section">
      <div class="debug-head">THIS DEVICE</div>
      ${rows('name', DeviceIdentity.name, { copy: true })}
      ${rows('id', DeviceIdentity.deviceId, { mono: true, copy: true })}
      ${rows('platform', DeviceIdentity.platform)}
    </section>
  `;
}

function wireDebugCopies(root) {
...
// ---- Devium: Self-hosted signaling sheet ----------------------------

function openSelfHostSheet() {
  let dlg = document.getElementById('modal-selfhost');
  if (!dlg) {
    dlg = document.createElement('dialog');
    dlg.id = 'modal-selfhost';
    dlg.className = 'sheet selfhost-sheet';
    dlg.innerHTML = `
      <form method="dialog" class="sheet-form">
        <header class="sheet-head">
          <h2 class="sheet-title">Self-hosted signaling</h2>
          <button class="sheet-close" value="cancel" aria-label="Close">✕</button>
        </header>
        <p class="sheet-sub">Signaling only introduces peers. Files always travel P2P over WebRTC — never through this server.</p>

        <label class="selfhost-label" for="selfhost-url">SIGNALING URL</label>
        <input type="url" id="selfhost-url" class="selfhost-input" spellcheck="false" autocomplete="off"/>
        <div class="selfhost-hint" id="selfhost-hint"></div>

        <div class="selfhost-snippet">
          <code>docker run -p 3000:3000 ghcr.io/poof/signal:latest</code>
          <button class="debug-copy" data-copy="docker run -p 3000:3000 ghcr.io/poof/signal:latest">⧉</button>
        </div>

        <div class="sheet-foot">
          <button class="poof-btn ghost" value="reset" id="selfhost-reset">Reset</button>
          <button class="poof-btn" value="save" id="selfhost-save">Save & reload</button>
        </div>
      </form>
    `;
    document.body.appendChild(dlg);
    dlg.querySelector('.sheet-close').addEventListener('click', () => dlg.close('cancel'));
    dlg.querySelector('#selfhost-save').addEventListener('click', (e) => {
      e.preventDefault();
      const v = dlg.querySelector('#selfhost-url').value;
      if (updateSignalingURLAndReload(v)) dlg.close('save');
    });
    dlg.querySelector('#selfhost-reset').addEventListener('click', (e) => {
      e.preventDefault();
      resetSignalingURL();
      dlg.close('reset');
    });
  ...
  dlg.querySelector('#selfhost-url').value = SIGNALING_URL;
  const hint = dlg.querySelector('#selfhost-hint');
  hint.textContent = airGapState().enabled
    ? '⚠ Air-gap is on — only LAN URLs will be accepted.'
    : '';
  hint.style.color = airGapState().enabled ? '#FFBF3C' : '';
  dlg.showModal();
}

// ---- Business: Workspace sheet ---------------------------------------

function openWorkspaceSheet() {
  let dlg = document.getElementById('modal-workspace');
  if (!dlg) {
    dlg = document.createElement('dialog');
    dlg.id = 'modal-workspace';
    dlg.className = 'sheet workspace-sheet';
    dlg.innerHTML = `
      <form method="dialog" class="sheet-form">
        <header class="sheet-head">
        ...
        <div class="ws-preview" id="ws-preview">
          <div class="ws-preview-logo" id="ws-preview-logo">P</div>
          <div class="ws-preview-text">
            <div class="ws-preview-name" id="ws-preview-name">Your workspace</div>
            <div class="ws-preview-sub">Live preview</div>
          </div>
        </div>

        <label class="ws-label" for="ws-name">WORKSPACE NAME</label>
        <input type="text" id="ws-name" class="selfhost-input" placeholder="Acme Corp" spellcheck="false" autocomplete="off"/>

        <label class="ws-label" for="ws-logo">LOGO (1–2 CHARACTERS)</label>
        <input type="text" id="ws-logo" class="selfhost-input" maxlength="2" spellcheck="false"/>

        <label class="ws-label">ACCENT COLOR</label>
        <div class="ws-swatches" id="ws-swatches"></div>

        <div class="sheet-foot">
          <button class="poof-btn" value="save" id="ws-save">Save</button>
        </div>
      </form>
    `;
    document.body.appendChild(dlg);
    dlg.querySelector('.sheet-close').addEventListener('click', () => dlg.close('cancel'));
    dlg.querySelector('#ws-save').addEventListener('click', (e) => {
      e.preventDefault();
      Workspace.setName(dlg.querySelector('#ws-name').value);
      Workspace.setLogo(dlg.querySelector('#ws-logo').value);
      toast('Workspace saved');
      dlg.close('save');
      renderTierUI();
    });
    dlg.querySelector('#ws-name').addEventListener('input', () => updateWsPreview(dlg));
    dlg.querySelector('#ws-logo').addEventListener('input', () => updateWsPreview(dlg));
  }
  dlg.querySelector('#ws-name').value = Workspace.getName();
  dlg.querySelector('#ws-logo').value = Workspace.getLogo();
  const swatches = dlg.querySelector('#ws-swatches');
  const current = Workspace.getColor();
  swatches.innerHTML = Workspace.COLOR_PRESETS.map((c) => `
    <button type="button" class="ws-swatch ${c.toLowerCase() === current.toLowerCase() ? 'selected' : ''}"
            data-color="${c}" style="background:${c}"></button>
  `).join('');
  swatches.querySelectorAll('[data-color]').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      e.preventDefault();
      Workspace.setColor(btn.dataset.color);
      swatches.querySelectorAll('.ws-swatch').forEach((el) => el.classList.remove('selected'));
...
function updateWsPreview(dlg) {
  const name = dlg.querySelector('#ws-name').value || 'Your workspace';
  const logo = dlg.querySelector('#ws-logo').value || Workspace.DEFAULT_LOGO;
  const color = Workspace.getColor();
  dlg.querySelector('#ws-preview-name').textContent = name;
  dlg.querySelector('#ws-preview-logo').textContent = logo;
  dlg.querySelector('#ws-preview-logo').style.background =
    `linear-gradient(135deg, ${color}, ${color}b3)`;
}

// ---- Business: Live audit sheet --------------------------------------

let _auditRepaint = null;

function openAuditSheet() {
  let dlg = document.getElementById('modal-audit');
  if (!dlg) {
    dlg = document.createElement('dialog');
    dlg.id = 'modal-audit';
    dlg.className = 'sheet audit-sheet';
    dlg.innerHTML = `
      <form method="dialog" class="sheet-form">
        <header class="sheet-head">
          <h2 class="sheet-title">Live audit</h2>
          <button class="sheet-close" value="cancel" aria-label="Close">✕</button>
        </header>
        <div class="audit-toolbar">
          <div class="audit-filters" id="audit-filters">
            <button type="button" data-f="all" class="audit-chip selected">All</button>
            <button type="button" data-f="sent" class="audit-chip">Sent</button>
            <button type="button" data-f="received" class="audit-chip">Received</button>
          </div>
          <button type="button" id="audit-export" class="poof-btn ghost">Export CSV</button>
        </div>
    ...
    document.body.appendChild(dlg);
    dlg.querySelector('.sheet-close').addEventListener('click', () => dlg.close('cancel'));
    dlg.dataset.filter = 'all';
    dlg.querySelector('#audit-filters').addEventListener('click', (e) => {
      const btn = e.target.closest('[data-f]');
      if (!btn) return;
      dlg.dataset.filter = btn.dataset.f;
      dlg.querySelectorAll('.audit-chip').forEach((c) => c.classList.remove('selected'));
      btn.classList.add('selected');
      paintAudit(dlg);
    });
    dlg.querySelector('#audit-export').addEventListener('click', (e) => {
      e.preventDefault();
      exportAuditCSV(dlg.dataset.filter || 'all');
    });
    dlg.addEventListener('close', () => {
      if (_auditRepaint) { clearInterval(_auditRepaint); _auditRepaint = null; }
    });
  }
  paintAudit(dlg);
  if (_auditRepaint) clearInterval(_auditRepaint);
  _auditRepaint = setInterval(() => paintAudit(dlg), 1500);
  dlg.showModal();
}

function auditEntries(filter) {
  const out = [];
  if (filter !== 'received') {
    for (const [id, s] of sentFiles) {
      out.push({ id: 's-' + id, direction: 'sent', name: s.name, size: s.size,
                 peer: peerNameOf(s.peerId), date: s.deliveredAt || s.seenAt || Date.now(),
                 receipt: s.receipt });
    }
  }
  if (filter !== 'sent') {
    for (const f of (transfer.receivedFiles || [])) {
      out.push({ id: 'r-' + (f.id || f.name), direction: 'received', name: f.name,
                 size: f.size || 0, peer: '—', date: f.date || Date.now(), receipt: null });
    }
  }
...
function peerNameOf(id) {
  return (store.peers.find((p) => p.id === id)?.name) || id || '—';
}

function paintAudit(dlg) {
  const filter = dlg.dataset.filter || 'all';
  const entries = auditEntries(filter);
  const body = dlg.querySelector('#audit-body');
  if (entries.length === 0) {
    body.innerHTML = `<div class="audit-empty"><div class="audit-empty-icon">📄</div><div>No activity yet.<br/>Your transfers will land here in real time.</div></div>`;
    return;
  }
  body.innerHTML = entries.map((e) => {
    const arrow = e.direction === 'sent' ? '↗' : '↙';
    const color = e.direction === 'sent' ? '#8B7EFF' : '#3FBF7F';
    const meta = [
      e.direction === 'sent' ? `to ${escapeHtml(e.peer)}` : 'received',
      e.receipt ? `· ${e.receipt}` : '',
      `· ${formatBytes(e.size || 0)}`
    ].filter(Boolean).join(' ');
    return `
      <div class="audit-row">
        <div class="audit-arrow" style="color:${color};background:${color}22">${arrow}</div>
        <div class="audit-row-body">
          <div class="audit-name">${escapeHtml(e.name)}</div>
          <div class="audit-meta">${meta}</div>
        </div>
        <div class="audit-time">${relativeTime(e.date)}</div>
      </div>`;
  }).join('');
}
...
function exportAuditCSV(filter) {
  const entries = auditEntries(filter);
  if (entries.length === 0) { toast('Nothing to export'); return; }
  const rows = ['timestamp,direction,name,size_bytes,peer,receipt'];
  for (const e of entries) {
    const iso = new Date(e.date).toISOString();
    const name = String(e.name).replace(/"/g, '""');
    const peer = String(e.peer).replace(/"/g, '""');
    rows.push(`${iso},${e.direction},"${name}",${e.size},"${peer}",${e.receipt || ''}`);
  }
  const blob = new Blob([rows.join('\n')], { type: 'text/csv' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `poof-audit-${Math.floor(Date.now() / 1000)}.csv`;
...
// ---- Business: Team management sheet ---------------------------------

function openTeamSheet() {
  let dlg = document.getElementById('modal-team');
  if (!dlg) {
    dlg = document.createElement('dialog');
    dlg.id = 'modal-team';
    dlg.className = 'sheet team-sheet';
    dlg.innerHTML = `
      <form method="dialog" class="sheet-form">
        <header class="sheet-head">
          <h2 class="sheet-title">Team</h2>
          <button class="sheet-close" value="cancel" aria-label="Close">✕</button>
        </header>
        <button type="button" id="team-invite" class="team-invite">
          <span class="team-invite-icon">+</span>
          <span class="team-invite-body">
            <div class="team-invite-title">Invite a teammate</div>
            <div class="team-invite-sub">Share a pair code from Settings</div>
          </span>
        </button>
    ...
    document.body.appendChild(dlg);
    dlg.querySelector('.sheet-close').addEventListener('click', () => dlg.close('cancel'));
    dlg.querySelector('#team-invite').addEventListener('click', (e) => {
      e.preventDefault();
      dlg.close('invite');
      openPairing();
    });
...
function paintTeam(dlg) {
  const list = dlg.querySelector('#team-list');
  const peers = store.peers || [];
  if (peers.length === 0) {
    list.innerHTML = `<div class="team-empty">No teammates yet.</div>`;
    return;
  }
  list.innerHTML = peers.map((p) => {
    const online = state.online.has(p.id);
    const role = Workspace.getRole(p.id);
    return `
      <div class="team-row" data-peer="${escapeHtml(p.id)}">
        <div class="team-row-head">
          <span class="team-dot" style="background:${online ? '#3FBF7F' : '#8B95A7'}"></span>
          <span class="team-name" data-role="name">${escapeHtml(p.name)}</span>
          <span class="team-platform">· ${escapeHtml(p.platform)}</span>
          <button type="button" class="team-edit" data-action="rename">✎</button>
        </div>
        <div class="team-row-actions">
          ${Workspace.ROLES.map((r) => `
            <button type="button" class="team-role ${r === role ? 'selected' : ''}" data-role="${r}">${r}</button>
          `).join('')}
          <button type="button" class="team-unpair" data-action="unpair">Unpair</button>
        </div>
      </div>`;
  }).join('');

  list.querySelectorAll('.team-row').forEach((rowEl) => {
    const peerId = rowEl.dataset.peer;
    rowEl.querySelectorAll('[data-role]').forEach((btn) => {
      if (!btn.dataset.role || btn.dataset.role === 'name') return;
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        Workspace.setRole(peerId, btn.dataset.role);
        paintTeam(dlg);
        renderTierUI();
      });
    });
    rowEl.querySelector('[data-action="rename"]').addEventListener('click', (e) => {
      e.preventDefault();
      const current = store.peers.find((p) => p.id === peerId)?.name || '';
      const next = prompt('Rename teammate', current);
      if (next && next.trim()) {
        store.rename?.(peerId, next.trim());
        paintTeam(dlg);
        renderTierUI();
        renderDevices();
      }
    });
    rowEl.querySelector('[data-action="unpair"]').addEventListener('click', (e) => {
      e.preventDefault();
      if (!confirm('Unpair this teammate?')) return;
      store.remove(peerId);
      signaling.broadcastUnpair?.(peerId);
...
// ─── Push Clipboard ──────────────────────────────────────────
$('#btn-push-clipboard').addEventListener('click', async () => {
  ...
  try {
    ...
    toast(`Clipboard push failed: ${e.message}`);
  }
});
...
function renderTierUI() {
  ...
  renderTierSection(tier, tierSectionEl, {
    ...
    familyDrop,
    multiDrop: familyDrop,
    airGapState: airGapState(),
    onAirGapToggle: setAirGapEnabled,
    openDebugConsole: openDebugConsoleSheet,
    openSelfHost: openSelfHostSheet,
    openWorkspace: openWorkspaceSheet,
    openAudit: openAuditSheet,
    openTeam: openTeamSheet,
    workspaceState: Workspace.workspaceState(),
    signalingURL: SIGNALING_URL,
    connectionState: manager?.state || (manager?.isReady ? 'connected' : (signalingConnected ? 'waiting for peer…' : 'offline')),
    activePeerId: state.connectedPeerId || null,
